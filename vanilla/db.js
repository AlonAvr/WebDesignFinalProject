// Vanilla JS Cost Manager database, backed only by localStorage.
// Exposes a single global: window.db = { openCostsDB }
// Must remain fully synchronous, dependency-free, and framework-free so it
// can be loaded directly via <script src="db.js"></script> and tested by
// automated university tests through window.db.

(function (global) {
  'use strict';

  var SUPPORTED_CURRENCIES = ['USD', 'ILS', 'GBP', 'EURO'];
  var RATES_KEY = 'costManagerExchangeRates';
  ...
  function readExchangeRates() {
    try {
      var raw = global.localStorage.getItem(RATES_KEY);
  ...
  function readCosts(databaseName) {
    try {
      var raw = global.localStorage.getItem(costsStorageKey(databaseName));
      if (!raw) return [];
      var parsed = JSON.parse(raw);
  ...
  function writeCosts(databaseName, costs) {
    global.localStorage.setItem(costsStorageKey(databaseName), JSON.stringify(costs));
  }
  ...
  function convert(amount, fromCurrency, toCurrency, rates) {
    ...
    var converted = amountInUsd * rates[toCurrency];
  ...
  function validateCost(cost) {
    ...
    if (typeof cost.currency !== 'string' || SUPPORTED_CURRENCIES.indexOf(cost.currency) === -1) {
  ...
  function openCostsDB(databaseName, databaseVersion) {
    ...
    return {
      ...
      addCost: function (cost) {
        ...
        var costs = readCosts(databaseName);
        costs.push(record);
      ...
      getReport: function (currency, year, month) {
        if (typeof currency !== 'string' || SUPPORTED_CURRENCIES.indexOf(currency) === -1) {
        ...
        var rates = readExchangeRates();
        var allCosts = readCosts(databaseName);

        var matching = allCosts.filter(function (item) {
          return item.date.year === targetYear && item.date.month === targetMonth;
        });

        var totalSum = 0;
        var reportCosts = matching.map(function (item) {
